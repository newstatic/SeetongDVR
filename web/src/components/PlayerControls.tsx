...
export function PlayerControls({
  ...
  const progressPercent = duration > 0
    ? Math.min(100, Math.max(0, ((currentTime - startTime) / duration) * 100))
    : 0;
  ...
  // 计算当前播放偏移量（限制在 0 到 duration 范围内）
  const rawOffset = currentTime - startTime;
  const currentOffset = duration > 0
    ? Math.min(duration, Math.max(0, rawOffset))
    : 0;
